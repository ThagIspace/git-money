...
const LastMonth = () => {
    ...
    useEffect(() => {
        ...
        const month = currentDate.getMonth() === 0 ? 11 : currentDate.getMonth() - 1; // Lấy tháng trước
        const year = currentDate.getMonth() === 0 ? currentDate.getFullYear() - 1 : currentDate.getFullYear();

        setLastMonthIncomes(incomes.filter(
            (income) => new Date(income.date).getMonth() === month && new Date(income.date).getFullYear() === year
        ));
        setLastMonthExpenses(expenses.filter(
            (expense) => new Date(expense.date).getMonth() === month && new Date(expense.date).getFullYear() === year
        ));
    }, [incomes, expenses]);

    const totalIncome = lastMonthIncomes.reduce((acc, income) => acc + income.amount, 0);
    const totalExpense = lastMonthExpenses.reduce((acc, expense) => acc + expense.amount, 0);
